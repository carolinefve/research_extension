...
async function loadAnalysis() {
  try {
    ...
    if (analysisId === "latest") {
      ...
      // Load specific analysis by timestamp
      const { analyses = [] } = await chrome.storage.local.get("analyses");
...
// Export functionality - Updated to generate PDF
...
function generatePDF(analysis) {
  ...
  addText("RESEARCH GAPS & FUTURE WORK", 14, true);
  addSpacing(8);
  ...
  // Save the PDF with title + date filename (Option 3)
